...
{
  START_LOG("write_equation_systems()", "MeshOutput");
  // FIXME: This should be unified with the version accepting system_names only.
  ...
  this->_build_variable_names_and_systems_vector(es, vec, names, system_vectors);
  //es.build_variable_names  (names);
  //es.build_solution_vector (soln);

  this->write_nodal_data (fname, vec, names);
...
template <class MT>
void MeshOutput<MT>::
_build_variable_names_and_systems_vector (const EquationSystems& es,
				  std::vector<Number>& vec,
				  std::vector<std::string>& names,
				  const std::map<std::string,const NumericVector<Number>*>* system_vectors)
{
  ...
  for (std::map<std::string,const NumericVector<Number>*>::const_iterator it = system_vectors->begin(); it != system_vectors->end();++it) system_names.insert(it->first);
  es.build_variable_names  (names, NULL, &system_names);
  es.build_systems_vector (vec, system_vectors);

  // For now, if we're doing a parallel format we're going to broadcast the vector from processor 0
  // to all of the processors to mimic what build_solution_vector used to do.
  // this is TERRIBLE and WASTEFUL but it's only temporary until we redesign the output of build_vector
  // and the inputs to the I/O... both of which should actually be NumericVectors....
  if(_is_parallel_format)
    {
