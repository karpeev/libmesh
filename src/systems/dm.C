...
unsigned int DM::MGCountLevelsFAC(MeshBase & mesh)
{
...
void DM::build_interpolation(EquationSystems & es_fine, EquationSystems & es_coarse, std::string system_name, SparseMatrix<Number>& Interpolation, NumericVector<Number> & fine_level, NumericVector<Number> & coarse_level)
{
  ...
  */
  ...
  // FIXME: fill out sparsity (which is basically std::vector<std::vector<dof_id_type> >) instead of or in addition to nz, nnz.
  // This will allow us to make this independent of the specific SparseMatrix specialization.
  SparsityPattern::Graph sparsity;
  std::vector<unsigned int> nz;
  std::vector<unsigned int> nnz;
  nz.resize(fine_level.local_size());
  nnz.resize(fine_level.local_size());
  sparsity.resize(fine_level.local_size());
  ...
  for (unsigned int var=0; var<n_variables; var++)
    {
      ...
      for (ConstElemRange::const_iterator elem_it=range.begin(); elem_it != range.end(); ++elem_it)
	    ...
	    elem_parent = elem;
          ...
          if (!elem_coarse->active()) // if element_coarse not active on coarse mesh, then elem is
		...
		if (fine_dof_indices[j] <= dof_map_fine.last_dof() && fine_dof_indices[j] >= dof_map_fine.first_dof())
		  {
			...
			std::cout << "Warning! Interpolation matrix created an empty row!\n";
		    }
		    else
		      {
  ...
  libMesh::out << "\n\n\nTotal Count: " << total_count << ", skip count: " << skip_count << "\n\n\n";
  w(10);
  ...
  std::cout << "Zeroes local: " << zero_count << ", processor: " << processor_id << "\n";
  ...
  Interpolation.init(fine_level.size(), coarse_level.size(), fine_level.local_size(), coarse_level.local_size(),sparsity);
...
void DM::flag_elements_uniformly(MeshBase & _mesh)
{
